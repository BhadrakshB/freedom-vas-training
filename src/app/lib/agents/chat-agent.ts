...
export class ChatAgent {
    ...
    async chat(input: ChatInput): Promise<ChatOutput> {
        try {
            ...
            const prompt = await this.buildChatPrompt(input, analytics);
     ...
     */
    private async getUserAnalytics(userId: string): Promise<UserSessionAnalytics> {
        ...
        const performanceTrends = this.calculatePerformanceTrends(completedSessions);
    ...
    private calculateAverageScores(sessions: CompletedSession[]): ScoringMetrics {
        ...
        const totals = sessions.reduce((acc, session) => ({
            policy_adherence: acc.policy_adherence + session.finalScores.policy_adherence,
            ...
            escalation_judgment: acc.escalation_judgment + session.finalScores.escalation_judgment,
            time_efficiency: acc.time_efficiency + session.finalScores.time_efficiency
        }), {
            policy_adherence: 0,
            empathy_index: 0,
            completeness: 0,
        ...
        return {
            ...
            escalation_judgment: Math.round(totals.escalation_judgment / sessions.length),
     ...
     */
    private calculatePerformanceTrends(sessions: CompletedSession[]): UserSessionAnalytics['performanceTrends'] {
     ...
     */
    private analyzeCommonScenarios(sessions: CompletedSession[]): { scenario: string; count: number; avgScore: number }[] {
     ...
     */
    private calculateRecentActivity(sessions: CompletedSession[]): UserSessionAnalytics['recentActivity'] {
        if (sessions.length === 0) {
        ...
        const sessionsThisWeek = sessions.filter(s => s.completedAt >= oneWeekAgo).length;
        const sessionsThisMonth = sessions.filter(s => s.completedAt >= oneMonthAgo).length;

        const lastSession = sessions.sort((a, b) => b.completedAt.getTime() - a.completedAt.getTime())[0];
     ...
     */
    private async buildChatPrompt(input: ChatInput, analytics: UserSessionAnalytics): Promise<string> {
        ...
        const trainingAssistantPrompt = `
You're Maya, the friendly training coach for Freedomvas. You've helped hundreds of virtual assistants improve their STR communication skills through realistic scenario training.
...
When responding:
- Start with a natural greeting that matches the time of day ("Morning!", "Afternoon!", "Evening!")
- Reference their specific situation without mentioning "trainingStatus" or other technical terms
...
- Keep responses to 2-3 sentences unless they ask for details
- End with an open question or clear next step

Examples of natural responses:

For new users:
"Hey there! 👋 Ready to practice handling real guest situations? I've got a quick check-in scenario that'll help you get comfortable with our system. Want to give it a try?"

For returning users:
"Great to see you back! Last time, you handled that frustrated guest really well - especially when you apologized sincerely before offering solutions. Ready to tackle something a bit more challenging today?"

For completed sessions:
"Wow, you've really improved your policy knowledge since last week! I especially liked how you explained the late check-out fee without sounding robotic. What would you like to work on next?"
...
Instead of describing what you'll do, just DO it naturally. Your goal is to make the trainee forget they're talking to AI - you're their supportive training partner.

User message: "${userMessage}"
`;

        return trainingAssistantPrompt;
    }
...
Area for Improvement: ${weakestSkill.skill} (${weakestSkill.score}/100)`;
        ...
        if (analytics.recentActivity.lastSessionDate) {
            const daysSinceLastSession = Math.floor(
                (new Date().getTime() - analytics.recentActivity.lastSessionDate.getTime()) / (1000 * 60 * 60 * 24)
            );
...
Sessions This Month: ${analytics.recentActivity.sessionsThisMonth}`;
        ...
        if (analytics.performanceTrends.length > 0) {
            const improvingSkills = analytics.performanceTrends.filter(t => t.trend === 'improving');
            const decliningSkills = analytics.performanceTrends.filter(t => t.trend === 'declining');

            if (improvingSkills.length > 0) {
                context += `\nImproving Skills: ${improvingSkills.map(s => s.dimension).join(', ')}`;
            }
            if (decliningSkills.length > 0) {
                context += `\nNeeds Attention: ${decliningSkills.map(s => s.dimension).join(', ')}`;
            }
        }
     ...
     */
    private getStrongestSkill(scores: ScoringMetrics): { skill: string; score: number } {
        const skills = [
        ...
        return skills.reduce((max, current) => current.score > max.score ? current : max);
    }

    /**
     * Get weakest skill from scores
     */
    private getWeakestSkill(scores: ScoringMetrics): { skill: string; score: number } {
        const skills = [
            { skill: 'Policy Adherence', score: scores.policy_adherence },
            { skill: 'Empathy', score: scores.empathy_index },
            { skill: 'Completeness', score: scores.completeness },
        ...
        return skills.reduce((min, current) => current.score < min.score ? current : min);
    }

    /**
     * Parse response and extract structured data
     */
    private parseResponse(responseText: string, analytics: UserSessionAnalytics): ChatOutput {
        // Extract suggested actions if present
        const suggestedActions: string[] = [];
        const actionMatches = responseText.match(/(?:try|consider|you could|i suggest|recommendation):?\s*([^.!?]+)/gi);
        if (actionMatches) {
            actionMatches.slice(0, 3).forEach(match => {
                const action = match.replace(/^(?:try|consider|you could|i suggest|recommendation):?\s*/i, '').trim();
                if (action.length > 10) {
        ...
        if (analytics.totalSessions > 0) {
            ...
            const improvingCount = analytics.performanceTrends.filter(t => t.trend === 'improving').length;
            const decliningCount = analytics.performanceTrends.filter(t => t.trend === 'declining').length;
    ...
    private generateRecommendations(analytics: UserSessionAnalytics): string[] {
        ...
        if (analytics.totalSessions < 5) {
            recommendations.push("Complete more training sessions to build consistency");
        }

        const weakestSkill = this.getWeakestSkill(analytics.averageScores);
        if (weakestSkill.score < 70) {
            recommendations.push(`Focus on improving ${weakestSkill.skill.toLowerCase()} skills`);
        }

        if (analytics.recentActivity.sessionsThisWeek === 0) {
            recommendations.push("Try to practice at least once this week to maintain momentum");
        }

        const decliningSkills = analytics.performanceTrends.filter(t => t.trend === 'declining');
        if (decliningSkills.length > 0) {
            recommendations.push(`Review fundamentals for ${decliningSkills[0].dimension.replace('_', ' ')}`);
        }

        if (analytics.commonScenarios.length > 0) {
            const lowPerformingScenario = analytics.commonScenarios.find(s => s.avgScore < 70);
            if (lowPerformingScenario) {
                recommendations.push(`Practice more ${lowPerformingScenario.scenario.toLowerCase()} scenarios`);
            }
        }
     ...
     */
    async compareRecentSessions(userId: string, sessionCount: number = 3): Promise<string> {
        ...
        recentSessions.forEach((session, index) => {
            const overallScore = this.calculateOverallScore(session.finalScores);
            const daysAgo = Math.floor((new Date().getTime() - session.completedAt.getTime()) / (1000 * 60 * 60 * 24));

            comparison += `${index + 1}. ${session.scenario.title} (${daysAgo} days ago)\n`;
            comparison += `   Overall Score: ${overallScore}/100\n`;
            comparison += `   Duration: ${Math.round(session.duration / 60000)} minutes\n\n`;
        });
