...
export interface TrainingActions {
  ...
  // Getters
  getTrainingState: (id: string) => TrainingState | undefined;
  getActiveTrainingState: () => TrainingState | undefined;
  getAllTrainingStates: () => TrainingState[];
  getActiveTrainingStates: () => TrainingState[];
  getArchivedTrainingStates: () => TrainingState[];
...
export function TrainingProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateTrainingState = useCallback(
    (id: string, updater: (state: TrainingState) => TrainingState) => {
  ...
  const getTrainingState = useCallback(
    (id: string): TrainingState | undefined => {
      return trainingStates.get(id);
    },
    [trainingStates]
  );

  const getActiveTrainingState = useCallback((): TrainingState | undefined => {
    return activeTrainingId ? trainingStates.get(activeTrainingId) : undefined;
  }, [activeTrainingId, trainingStates]);

  const getAllTrainingStates = useCallback((): TrainingState[] => {
    return Array.from(trainingStates.values()).sort(
      (a, b) => b.lastActivity.getTime() - a.lastActivity.getTime()
    );
  }, [trainingStates]);

  const getActiveTrainingStates = useCallback((): TrainingState[] => {
    return Array.from(trainingStates.values())
      .filter((state) => !state.isArchived)
      .sort((a, b) => b.lastActivity.getTime() - a.lastActivity.getTime());
  }, [trainingStates]);

  const getArchivedTrainingStates = useCallback((): TrainingState[] => {
    return Array.from(trainingStates.values())
      .filter((state) => state.isArchived)
      .sort((a, b) => b.lastActivity.getTime() - a.lastActivity.getTime());
  }, [trainingStates]);
